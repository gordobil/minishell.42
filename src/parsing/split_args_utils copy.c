...
int	check_unclosed_copy(char *s)
{
	int	i;
...
int	jump_spaces_copy(char *s, int i)
{
	while (s[i] == ' ' || s[i] == '	')
		i++;
	return (i);
...
int	empty_quotes_copy(char *s, int i)
{
	int		start;
	char	q;
...
int	jump_empty_copy(char *s, int i)
{
	i = jump_spaces(s, i);
	i = empty_quotes(s, i);
	return (i);
...
int	all_same_quotes_copy(char *s)
{
	int		i;
	char	q;
	i = jump_spaces(s, 0);
